...
### 2026-06-11 Admin User Cleanup And Redis Job Visibility / 后台用户清理与真实任务可见性
- Clarified that many `smoke-*`, `ocr-*`, `office-*`, and `@example.com` accounts are generated by smoke-test scripts, then surfaced them in `/control-room` as `测试账号` instead of making them look like real customers.
- Reworked user operations from vague `启用/已验证` controls into production-facing actions: save role, ban/unban, and delete user. Current admin self-demotion, self-ban, and self-delete remain blocked by backend safeguards.
- Added `DELETE /api/v1/admin/users/{user_id}` with audit logging and safety checks.
